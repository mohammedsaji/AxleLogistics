document.addEventListener('DOMContentLoaded', function (event) {
    async function PayloadExtractor() {
        ...
        if (viewState === shipmentStates.CARGO.READ) {
            ...
            if (cargoId) {
                ...
                LayoutRenderer(cargoId, shipmentId, response);
            }
        }
    }
...
function LayoutRenderer(cargoId, shipmentId, response) {

    const viewState = localStorage.getItem("viewState");
    const cargoShellWrapperBody = document.querySelector('.cargo-shell-wrapper-body');

    if (cargoShellWrapperBody) {
        cargoShellWrapperBody.innerHTML = '';
    }

    Object.entries(response).forEach(([keyName, keyValue]) => {

        const elementDiv = document.createElement('div');
        elementDiv.className = 'cargo-shell-inner-body';
        ...
        cargoShellWrapperBody.append(elementDiv);
    });
    ...
    cargoShellWrapperBody.append(backToShipmentBtn);
...
function ClickEventBinder() {
    ...
    const backToShipmentListBtn = document.querySelector('.back-to-shipment-btn');
    if(backToShipmentListBtn){
        backToShipmentListBtn.addEventListener('click', function () {
